%% Script: Analyze and plot results of the calculated synchrony values of the experimental data
...
for idl1=1:size(methods,2)
    ...
    m=idl1; % index method
    method(m).name=methods{idl1};
    path2 = [path_results filesep methods{idl1}];
    list2 = dir(path2);
    ...
    for idl2=size(list2,1) %% only last BIC concentration!!!!
        ...
        method(m).exp(e).name=list2(idl2).name;
        ...
        for idl3=3:size(list3,1)
            ...
            method(m).exp(e).chip(c).name=list3(idl3).name;
            ...
            method(m).exp(e).chip(c).PARAMETER = temp.PARAMETER;
            clear temp
        end
    end
end
...
for m=1:size(method,2)
    for e=1
        ...
        REF_c=zeros(1,size(method(m).exp(e).chip,2));
        BIC_c=zeros(1,size(method(m).exp(e).chip,2));
        DIF_c=zeros(1,size(method(m).exp(e).chip,2));
        
        for c=1:size(method(m).exp(e).chip,2)
            
            % get mean parameter value for every chip
            REF_c(c) = method(m).exp(e).chip(c).PARAMETER(P_index).mean(1);
            BIC_c(c) = method(m).exp(e).chip(c).PARAMETER(P_index).mean(2);
            DIF_c(c) = BIC_c(c) / REF_c(c);
            
        end
        
        % calculate mean value of all chips
        method(m).REFmean(e) = mean(REF_c);
        method(m).BICmean(e) = mean(BIC_c);
        method(m).DIFmean(e) = mean(DIF_c);
        method(m).REF = REF_c;
        method(m).BIC = BIC_c;
        % calculate std value of all chips
        method(m).REFstd(e) = std(REF_c);
        method(m).BICstd(e) = std(BIC_c);
        method(m).DIFstd(e) = std(DIF_c);
        
        [pp,ppf,ppf_rel,validity]=pairedTest(BIC_c,REF_c,'right'); % Tail='both', test if BIC!=REF, Tail='right', test if BIC > REF
        method(m).pp(e)=pp;      % parameter dependend test
        method(m).validity(e)=validity;
        method(m).ppf(e)=ppf; % parameter free test
        method(m).ppf_rel(e)=ppf_rel; % parameter free test using relative values
    end
end
...
%% 3) Plot Sync-Values
if 1
    plotSyncValuesVsBic(method,methodNames)
end